...
#define SIG_APPLY_SCRIPT "sh /root/lionic/lcsig.sh"
...
int main(int argc, char **argv)
{
    ...
    swu_syslog(LOG_INFO, "ASD V3 Support: is_deleted checks\r\n");
    /* if "is_delted":"Y", then avoding download step. Update the corresponding error/msg */
    if(strcmp("Y", pMetadata->pIsDeleted) == 0) {
       ASDEBUG("\n %s(): is_deleted is '%s'. Can not perform downloading\n", __FUNCTION__,
        pMetadata->pIsDeleted);
       swu_syslog(LOG_ERR, " ASD V3 Support: is_deleted is '%s', Can not perform downloading.\r\n",
        pMetadata->pIsDeleted);
...
int checkUpdates(ASDParams_t * pASDParams, swtype_e eSWType)
{
    ...
    if (getMetaData(pASDParams, &pMetadata) != ASD_SUCCESS) {
	ASDEBUG("\n%s(): Failed to get Metadata.\n", __FUNCTION__);
	return ASD_FAILURE;
    }
...
#ifdef ASD_V3_SUPPORT
    if (pMetadata->pIsDeleted)
        free(pMetadata->pIsDeleted);
#endif
...
int getMetaData(ASDParams_t * pASDParams, Metadata_t ** pMetadat)
{
	...
	if (parseMetadata(data, &pMetadata) != ASD_SUCCESS) {
	    freeMetadata(pMetadata);
	    free(data);
	    freeEncodedData(pEncodUDI);
	    return ASD_FAILURE;
...
int parseMetadata(char *pData, Metadata_t ** pMetadata)
{
	...
	if (pMetadataResp) {
        ...
        pExceptionItem = cJSON_GetObjectItem(pMetadataResp, "asd_metadata_exception");
...
#ifdef ASD_V3_SUPPORT
        //swu_syslog(LOG_INFO, " ASD V3 Support: metadata_mdfid_list\r\n");
        pItem = cJSON_GetObjectItem(pMetadataResp, "metadata_mdfid_list");
        if (pItem == NULL) {
        ASDEBUG("\n%s(): failed to parse metadata_mdfid_list.\n",
            __FUNCTION__);
        return ASD_FAILURE;
        }
        pItem = cJSON_GetArrayItem(pItem, 0);
#else
...
#ifdef ASD_V3_SUPPORT
        //swu_syslog(LOG_INFO, " ASD V3 METADATA LST: %s .\n", __FUNCTION__, cJSON_Print(pItem));
        pItem = cJSON_GetObjectItem(pItem, "software_response_list");
        if ( pItem == NULL ) {
            ASDEBUG("\n%s(): failed to parse software_response_list.\n",
                    __FUNCTION__);
            return ASD_FAILURE;
        }
        pItem = cJSON_GetArrayItem(pItem, 0);
...
#ifdef ASD_V3_SUPPORT
        pIsDeleted = cJSON_GetObjectItem(pSubItem, "is_deleted");
        if (pIsDeleted == NULL) {
        swu_syslog(LOG_INFO, " ASD V3 Support: failed to parse 'is_deleted'\r\n");
        ASDEBUG("\n%s(): failed to parse is_deleted.\n",
            __FUNCTION__);
        return ASD_FAILURE;
        }
        getObjectValue(pIsDeleted, &pMtadata->pIsDeleted);
        swu_syslog(LOG_INFO, " ASD V3 Support: is_deleted is '%s'\r\n",pMtadata->pIsDeleted);
#endif
