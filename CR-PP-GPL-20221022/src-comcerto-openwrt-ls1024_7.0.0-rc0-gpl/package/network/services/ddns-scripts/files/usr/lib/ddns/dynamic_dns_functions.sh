...
get_current_ip()
{
	...
	ip_network=`uci get ddns.$service_id.interface`
	
	checkusb=`echo $ip_network | grep usb`>/dev/null 2>&1
	if [ -n "$checkusb" ];then
		usbproto=`uci get network.$ip_network.proto` >/dev/null 2>&1
		if [ "$usbproto" = "mobile" ]; then
			detect=$(cat /var/USBCONNSTATUS | awk -F: '{printf $4}')
			if [ "$detect" = "4G" ]; then
				ip_network=${ip_network}"_4"
			fi
		elif [ "$usbproto" = "qmi" ];then
			ip_network=${ip_network}"_4"
		fi 
	fi	
	...
	then
		if [ -n "$checkusb" ];then
			...
			network_get_wanip current_ip "$ip_network" || return
		fi
...
verbose_echo_iface()
{
	...
	echo "$1" >> /var/log/ddns.$iface.logs
}

start_daemon_for_all_ddns_sections()
{
	local event_interface="$1"
	event_interface=$(echo $event_interface | sed s/p//g)
	SECTIONS=""
	config_cb() 
	{
		SECTIONS="$SECTIONS $2"
