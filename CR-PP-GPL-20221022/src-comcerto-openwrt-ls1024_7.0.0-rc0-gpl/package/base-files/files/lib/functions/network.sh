...
{
	local cache
	eval cache=\$interface_physdev
	if [ -n "$cache" ]
	then
		eval $1=\$cache
	else
		network_get_physdev $1 $2
		eval interface_physdev=\$$1
	fi
}
...
network_get_mwaniface()
{
	...
	[ -n "$isUSB" ] && {
		...
		then
			...
			port=$(cat /var/USBCONNSTATUS | awk -F: '{printf $2}' | grep "ttyUSB")
			if [ "$port" = "" ];then
			    devpath="/sys/class/usbmisc/$devname/device/"
			    finalIfname="$(ls "$devpath"/net 2>/dev/null)"
			    eval "export -- \"$1=$finalIfname\""
			else
			    devpath="$(readlink -f /sys/class/tty/$devname/device)"
			    finalIfname="$( ls "$devpath"/../../*/net )"
			    eval "export -- \"$1=$finalIfname\""
			fi
		fi
		return
