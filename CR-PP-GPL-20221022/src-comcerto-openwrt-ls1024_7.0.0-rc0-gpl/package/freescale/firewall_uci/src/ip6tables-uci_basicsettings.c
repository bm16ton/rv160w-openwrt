...
void insert_lanvpn_http_rules6(struct ip6tc_handle *handle)
{
...
void insert_lanvpn_https_rules6(struct ip6tc_handle *handle)
{
    char final_lan_iface[16];
    char *tablename="filter";
    int ret;
...
int insert_lanvpn_management6(struct uci_context *ctx,struct ip6tc_handle *filter_handle)
{
    ...
    char *strptr1=strdup("firewall.global_configuration.lan_vpn_https");
        ...
        if(lan_vpn_http)
        {
            insert_lanvpn_http_rules6(filter_handle);

        }
        else if(lan_vpn_https)
        {
            insert_lanvpn_https_rules6(filter_handle);
        }
