...
void insert_lanvpn_http_rules(struct iptc_handle *handle)
{
    char final_lan_iface[16];
    char *tablename="filter";
    int ret;
...
void insert_lanvpn_https_rules(struct iptc_handle *handle)
{
    char final_lan_iface[16];
    char *tablename="filter";
    int ret;
...
{
    struct uci_ptr ptr;
    int lan_vpn_http=0,lan_vpn_https=0;
    ...
    char *strptr1=strdup("firewall.global_configuration.lan_vpn_https");
        ...
        if(lan_vpn_http)
        {
            insert_lanvpn_http_rules(filter_handle);
            insert_lanvpn_dnat(80,nat_handle);

        }
        if(lan_vpn_https)
        {
            insert_lanvpn_https_rules(filter_handle);
            insert_lanvpn_dnat(443,nat_handle);
        }
