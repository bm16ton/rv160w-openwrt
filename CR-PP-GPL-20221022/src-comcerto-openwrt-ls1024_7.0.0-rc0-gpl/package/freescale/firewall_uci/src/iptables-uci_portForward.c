...
//prototype:	strreplace(char *outputbuff,char * original_buff,char *search_string,char * replace_with);
		...
		{//Not all.
            ...
            memset(nat_reflection, '\0', sizeof(nat_reflection));
            sprintf(nat_reflection,"iptables-uci -t filter -I vlan_routing_hairpin_bypass -m mark --mark 0xee00 -p %s --dport %d:%d -j ACCEPT ", pf->protocol,pf->int_port_start, pf->int_port_end);
            tokenize_and_docommand(nat_reflection, "filter", handle_filter);
		}
		else
		{
            if(!(pf->int_port_start) && !(pf->int_port_end))
            {
                ...
                memset(nat_reflection, '\0', sizeof(nat_reflection));
                sprintf(nat_reflection,"iptables-uci -t filter -I vlan_routing_hairpin_bypass -m mark --mark 0xee00 -p udp -j ACCEPT ");
                tokenize_and_docommand(nat_reflection, "filter", handle_filter);

                memset(nat_reflection, '\0', sizeof(nat_reflection));
                sprintf(nat_reflection,"iptables-uci -t filter -I vlan_routing_hairpin_bypass -m mark --mark 0xee00 -p tcp -j ACCEPT ");
                tokenize_and_docommand(nat_reflection, "filter", handle_filter);
            }
			else
			{
                ...
                memset(nat_reflection, '\0', sizeof(nat_reflection));
                sprintf(nat_reflection,"iptables-uci -t filter -I vlan_routing_hairpin_bypass -m mark --mark 0xee00 -p tcp --dport %d:%d -j ACCEPT ", pf->int_port_start,pf->int_port_end);
                tokenize_and_docommand(nat_reflection, "filter", handle_filter);

                memset(nat_reflection, '\0', sizeof(nat_reflection));
                sprintf(nat_reflection,"iptables-uci -t filter -I vlan_routing_hairpin_bypass -m mark --mark 0xee00 -p udp --dport %d:%d -j ACCEPT ", pf->int_port_start,pf->int_port_end);
                tokenize_and_docommand(nat_reflection, "filter", handle_filter);
			}
		}
	}
...
//prototype:	strreplace(char *outputbuff,char * original_buff,char *search_string,char * replace_with);
				...
				else
				{
					if(!pf->int_port_start && !pf->int_port_end)
					{
                        ...
                        memset(nat_reflection, '\0', sizeof(nat_reflection));
                        sprintf(nat_reflection,"iptables-uci -t filter -I vlan_routing_hairpin_bypass -m mark --mark 0xee00 -p udp -j ACCEPT ");
                        tokenize_and_docommand(nat_reflection, "filter", handle_nat);
...
void config_portForwarding(struct uci_context *ctx)
{
		...
		&& iptc_is_chain(strdup("nat_reflection_out"), handle_nat))
	{
        ...
        if(!iptc_flush_entries(strdup("vlan_routing_hairpin_bypass"), handle_filter))
            printf("Failed in flushing rules of vlan_routing_hairpin_bypass in iptables\r\n");
	}
	else
