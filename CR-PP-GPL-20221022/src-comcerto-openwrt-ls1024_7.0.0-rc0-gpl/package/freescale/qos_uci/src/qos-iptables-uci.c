/*
 * qos-iptables-uci.c - sample code for the ucimap/libuci library
 * 
 */
#include "qos-iptables-uci.h"

#include "xtables.h"
#include "libiptc/libiptc.h"
#include "iptables-multi.h"

/*iptables -A egress_chain -t mangle $protocol$sport$dport$sip$dip$rcvint$outint$dscpval-m comment --comment "$1$2" -j QOSCONNMARK --set-xmark $queue/0xf0f0000000ffffff*/
#define EGRESS_IPTABLE_RULE(rcviface,outiface) do{\
		memset(commandBuff, '\0', sizeof(commandBuff));\
		sprintf(commandBuff,"qos-iptables-uci -A egress_chain -t mangle %s%s%s%s%s%s%s%s-m comment --comment %s%s"\
...
void tokenize_and_docommand(char *cmdBuff,char *table, struct iptc_handle *handle)
{
	...
	ret = do_command(newargc_g, newargv_g, &table, &handle);
	if (!ret)
...
void print_usage()
{
	fprintf(stderr, "USGAE: qos-iptables-uci <[ -dilf ]> <classname.flowname>\n"
			"			-d to enable debug.\n"
			"			-i configure iptables rules for QoS\n"
			"			-l configure lcsh rules for application QoS\n"
			...
			"	Note: options -i and -l are mutually exclusive.\n"
		);
	exit(1);
}
...
int add_rule_to_chain(char class_name[128],char flow_name[128], struct uci_context *ctx, struct iptc_handle *handle)
{
	char tuple[128]="";
	struct qos_flow flow = {"any","8","","","","","","",65,4,""};
...
//printf("For option:%s valueis:%s\r\n",o->e.name,o->v.string);
	...
	if(flow.ip_version == 6)
		return -1;   //This file is for only v4 handling. Hence the iptables rules are v4 only.

	/*Handling of common parameters*/
	//Handling protocol
	if(strcmp("icmp",flow.proto)==0)
		sprintf(protocol,"-p icmp --icmp-type %s ",flow.icmptype);
	else if (strcmp("tcp",flow.proto)==0 || strcmp("udp",flow.proto)==0)
	{
...
/*	else
	...
	if (strncmp(flow_name,"ingress",7)==0)
	{
		...
		{
			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i %s+ %s-m comment --comment %s%s "
			...
			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i pptp-wan1+ %s-m comment --comment %s%s "
					"-j QOSCONNMARK --set-mark 0x80A%s%s0000000000/0xffffffff00000000",
					protocol, sport, dport, sip, dip, dscp, class_name, flow_name, flow.queue, mark_dscp);
			tokenize_and_docommand(commandBuff, "mangle", handle);

			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i l2tp-wan1+ %s-m comment --comment %s%s "
					"-j QOSCONNMARK --set-mark 0x80A%s%s0000000000/0xffffffff00000000",
					protocol, sport, dport, sip, dip, dscp, class_name, flow_name, flow.queue, mark_dscp);
			tokenize_and_docommand(commandBuff, "mangle", handle);
	
			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i ppoe-wan1+ %s-m comment --comment %s%s "
					"-j QOSCONNMARK --set-mark 0x80A%s%s0000000000/0xffffffff00000000",
					protocol, sport, dport, sip, dip, dscp, class_name, flow_name, flow.queue, mark_dscp);
			tokenize_and_docommand(commandBuff, "mangle", handle);
		}
		else if (strncmp(flow.rcvinterface,"wan2",4)==0)
		{
			...
			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i pptp-wan2+ %s-m comment --comment %s%s "
					"-j QOSCONNMARK --set-mark 0x80B%s%s0000000000/0xffffffff00000000",
					protocol, sport, dport, sip, dip, dscp, class_name, flow_name, flow.queue, mark_dscp);
			tokenize_and_docommand(commandBuff, "mangle", handle);

			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i l2tp-wan2+ %s-m comment --comment %s%s "
					"-j QOSCONNMARK --set-mark 0x80B%s%s0000000000/0xffffffff00000000",
					protocol, sport, dport, sip, dip, dscp, class_name, flow_name, flow.queue, mark_dscp);
			tokenize_and_docommand(commandBuff, "mangle", handle);
	
			memset(commandBuff, '\0', sizeof(commandBuff));
			sprintf(commandBuff,"qos-iptables-uci -A ingress_chain -t mangle %s%s%s%s%s-i ppoe-wan2+ %s-m comment --comment %s%s "
					"-j QOSCONNMARK --set-mark 0x80B%s%s0000000000/0xffffffff00000000",
					protocol, sport, dport, sip, dip, dscp, class_name, flow_name, flow.queue, mark_dscp);
			tokenize_and_docommand(commandBuff, "mangle", handle);
		}
...
//printf("final_rcv_iface:%s\r\n",final_rcv_iface);
		...
		memset(commandBuff, '\0', sizeof(commandBuff));
		sprintf(commandBuff,"qos-iptables-uci -A class_chain -t mangle %s%s%s%s%s%s-o outint %s-m comment --comment %s%s"
		...
		{//This policy is of WAN1 interface.
			...
			if(queueconf.queue1)
			{
				...
				memset(commandBuff, '\0', sizeof(commandBuff));
				sprintf(commandBuff,"qos-iptables-uci -A egress_chain -t mangle %s%s%s%s%s%s-o %s+ %s-m comment --comment %s%s"
					" -j QOSCONNMARK --set-mark %s/0xf0f0000000ffffff", protocol, sport, dport, sip, dip,
				...
				EGRESS_IPTABLE_RULE(final_rcv_iface,"-o ppoe-wan1+ ");
				if(wifiboard && strlen(final_rcvwifi_iface))
					EGRESS_IPTABLE_RULE(final_rcvwifi_iface,"-o ppoe-wan1+ ");

				EGRESS_IPTABLE_RULE(final_rcv_iface,"-o pptp-wan1+ ");
				if(wifiboard && strlen(final_rcvwifi_iface))
					EGRESS_IPTABLE_RULE(final_rcvwifi_iface,"-o pptp-wan1+ ");

				EGRESS_IPTABLE_RULE(final_rcv_iface,"-o l2tp-wan1+ ");
				if(wifiboard && strlen(final_rcvwifi_iface))
					EGRESS_IPTABLE_RULE(final_rcvwifi_iface,"-o l2tp-wan1+ ");
			}
		...
		{//This policy is of WAN2 interface.
			...
			if(queueconf.queue1)
			{
				...
				EGRESS_IPTABLE_RULE(final_rcv_iface,"-o ppoe-wan2+ ");
				if(wifiboard && strlen(final_rcvwifi_iface))
					EGRESS_IPTABLE_RULE(final_rcvwifi_iface,"-o ppoe-wan2+ ");

				EGRESS_IPTABLE_RULE(final_rcv_iface,"-o pptp-wan2+ ");
				if(wifiboard && strlen(final_rcvwifi_iface))
					EGRESS_IPTABLE_RULE(final_rcvwifi_iface,"-o pptp-wan2+ ");

				EGRESS_IPTABLE_RULE(final_rcv_iface,"-o l2tp-wan2+ ");
				if(wifiboard && strlen(final_rcvwifi_iface))
					EGRESS_IPTABLE_RULE(final_rcvwifi_iface,"-o l2tp-wan2+ ");
			}
		...
		{//This policy is of USB1 interface.
			...
			if(findIface && strcmp(findIface->l3ifname,"null")!=0)
			{
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-1)); //Since we are in queue1
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-2)); //Since we are in queue2
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-3)); //Since we are in queue3
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-4)); //Since we are in queue4
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-5)); //Since we are in queue4
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-6)); //Since we are in queue4
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80C000000000%s0%d",mark_dscp,(n-7)); //Since we are in queue4

					EGRESS_IPTABLE_RULE(final_rcv_iface, finalL3ifname);
					if(wifiboard && strlen(final_rcvwifi_iface))
						EGRESS_IPTABLE_RULE(final_rcvwifi_iface, finalL3ifname);
				}
			} //Iface is an Active interface
		...
		{//This policy is of USB2 interface.
			...
			if(findIface && strcmp(findIface->l3ifname,"null")!=0)
			{
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-1)); //Since we are in queue1
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-2)); //Since we are in queue2
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-3)); //Since we are in queue3
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-4)); //Since we are in queue4
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-5)); //Since we are in queue4
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-6)); //Since we are in queue4
				...
				{
					memset(queue_value, '\0', sizeof(queue_value));
					sprintf(queue_value,"0x80D000000000%s0%d",mark_dscp,(n-7)); //Since we are in queue4
...
int main(int argc, char **argv)
{
	...
	struct uci_ptr ptr;
	bool config_chains = 0, config_lcsh = 0;
	char temp[]="systeminfo.sysinfo.pid";
	int i,c,y;

	//0. Process command line arguments
	while((c = getopt(argc, argv, "dilf:")) != -1) {
		switch(c) {
			case 'd':
				enable_debug = 1;
				break;
	...
	if (config_lcsh == 1 && config_chains == 1)
	{
		printf("Only one of the options -i or -l are allowed.\n");
		print_usage();
		return 1;
	}

	else if (config_lcsh == 0 && config_chains ==0)
	{
		printf("One of the options -i or -l is must.\n");
		print_usage();
		return 1;
	}

	//1. Init iptables related infra
	iptables_globals.program_name = "qos-iptables-uci";
	...
	else if (config_lcsh == 1)
	{
		printf("This feature is not implemented yet! Stay tuned for more updates\n");
