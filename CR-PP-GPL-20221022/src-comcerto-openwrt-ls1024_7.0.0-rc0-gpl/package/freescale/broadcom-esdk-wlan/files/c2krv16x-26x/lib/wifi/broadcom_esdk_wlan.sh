...
wlan24_led_off() {
	/usr/bin/rv16x_26x_led.sh wlan2.4 off
}

wlan24_led_on() {
	/usr/bin/rv16x_26x_led.sh wlan2.4 on
	/etc/init.d/wlan_led start radio0
}

wlan5g_led_off() {
	/usr/bin/rv16x_26x_led.sh wlan5g off
}

wlan5g_led_on() {
	/usr/bin/rv16x_26x_led.sh wlan5g on
...
disable_broadcom() {
	...
	(
		...
		if [ "$board" = "RV260W" -a "$device" = "wl0" ] || [ "$board" = "RV160W" -a "$device" = "wl1" ]; then
			wlan24_led_off
			if [ "$2" != "nolog" ];then
...
init_broadcom() {
	...
	mac_cur=$(uci get systeminfo.sysinfo.maclan 2>/dev/null)
	last_value=${mac_cur##*:}
	...
	while [ $vif_idx -lt $max_vif_idx ]; do
		...
		[ $vif_idx -eq 0 ] && {
			ifname="${device}"
			last_value_new="$(echo $(( (0x${last_value}  + ${base_increment}) & 0xf )))"
		}

		# add mac addr for multi-ssid
		[ $vif_idx -gt 0 ] && {
			ifname="${device}.${vif_idx}"
			last_value_new="$(echo $(( (0x${last_value}  + $vif_idx + ${base_increment}) & 0xf )))"
		}
		
		last_value_new=$(printf "%x" ${last_value_new})
		mac_new="${mac_cur:0:16}$last_value_new"
...
enable_broadcom() {
	...
	case "$pid_radio" in
		...
		I)
			[ "$band" = "b" ] && {
				...
				include_chanlist="36 40 44 48 52 56 60 64 149 153 157 161 165"
			}
			if [ "$board" = "RV260W" ];then
				country="IN/985"
			else
		...
		J)
			[ "$band" = "b" ] && {
				...
				include_chanlist="36 40 44 48 52 56 60 64 100 104 108 112 116 120 124 128 132 136 140"
			}
			if [ "$board" = "RV260W" ];then
				country="JP/937"
			else
	...
	wl -i "$device" country ${country:-US}
	...
	#	wl -i "$device" spect 0
		...
		if [ "$board" != "RV160W" -a "$vht_mask" = "1" ];then
	...
	[ -n "$rate_settings" ] && {
		...
		if [ "$vht_mask" = "1" ];then
			...
			while [  $_vif_idx -lt 4 ];do
				[ $_vif_idx -gt 0 ] && _vif_ifname="${device}.${_vif_idx}"
				[ -e "$_vif_ifname" ] || continue
				wl -i "$_vif_ifname" rateset -v 0x3ff 0x3ff 0x3ff
				_vif_idx=$(( $_vif_idx + 1 ))
			done
		fi
	}
	...
	[ "$band" = "a" ] && {
		case "$pid_radio" in
		A)
			[ "$board" = "RV160W" ] && wl -i "$device" radarthrs 0x6b4 0x30 0x6b4 0x30 0x6b4 0x30 0x6a0 0x20 0x6a0 0x30 0x6a0 0x20
			[ "$board" = "RV260W" ] && wl -i "$device" radarthrs 0x6ac 0x20 0x6ac 0x20 0x6ac 0x20 0x6ac 0x20 0x6ac 0x20 0x6ac 0x20
		;;
		E)
			[ "$board" = "RV160W" ] && wl -i "$device" radarthrs 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30
			[ "$board" = "RV260W" ] && wl -i "$device" radarthrs 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30 0x6ac 0x30
		;;
		esac
	}
	...
	[ -z "$txpower" ] || {
	wl -i $device pwr_percent 100
		if [ "$txpower" = "100" ]; then
			wl -i $device pwr_degrade 0
		elif [ "$txpower" = "50" ]; then
			wl -i $device pwr_degrade 12
		elif [ "$txpower" = "25" ]; then
			wl -i $device pwr_degrade 24
		elif [ "$txpower" = "12" ]; then
			wl -i $device pwr_degrade 36
		fi
	}
